...
df = pd.read_csv('output/parents_LLM_extraction_annot.csv')
# LLM structures that were missed, fold without the structure
# df = pd.read_csv('output/parents_LLM_extraction_v2_missed.csv')
df['substrate'] = ''
print(df)
seq_col = 'aa_sequence'
for label, seq, substrate in df[['id', seq_col, substrate_col]].values:
        if label == 'chai86':
                run_chai(label, seq, substrate, output_dir)
